...
- [ ] 0.5 Review and approve the proposal, design, route map, task plan, and
...
- [ ] 3.2 Add one idempotent source-scoped copy service that authorizes the
...
- [ ] 3.3 Add `POST .../rp-configurations/{sourceUuid}/copy` with target
  ...
  errors.
- [ ] 3.4 Centralize and test the reviewed reusable-answer allowlist. Exclude
...
- [ ] 3.5 Add the focused `/copy` form for Test, Staging, and Production
...
- [ ] 3.6 Make `Copy configuration` a secondary selected-record lifecycle
  action rather than a task-hub card. Remove Promote/Progress/next-environment
  product language from the canonical path.
- [ ] 3.7 Record a minimized copy audit event and prove copied values, secrets,
  tokens, personal information, and raw provider data are absent from logs and
  audit payloads.
- [ ] 3.8 Add source/target environment matrix, same-environment, repeated
...
- [ ] 5.1 Replace the one-item Partner work disclosure with a direct
  `Partner workspaces` `GcdsNavLink` and update active-parent and route-catalog
  tests.
- [ ] 5.2 Remove the optional verbose Partner-work close trigger and all
  translations/tests that require a nested `Close Partner work menu` label.
- [ ] 5.3 Keep the display name and compact active workspace/role context in
...
- [ ] 5.4 Ensure only one disclosure is open and dismiss it on Escape, sibling
...
- [ ] 5.5 Preserve the standard GCDS mobile root Menu/Close behavior and prove a
  Close control is present only while its corresponding panel is open.
- [ ] 5.6 Replace GCDS mocks where necessary with real-component integration or
...
- [ ] 5.7 Verify below 768 px, 768-1023 px, 1024 px and wider, 200 percent zoom,
  keyboard-only use, visible focus, and long English/French labels.

## 6. Contracts, content, and documentation

- [ ] 6.1 Update English/French translations, route metadata, breadcrumbs,
  headings, hints, statuses, copy exclusions, error messages, success content,
  and accessible names together.
- [ ] 6.2 Update OpenAPI and checked frontend contract types for the copy API
  and any focused access endpoints.
- [ ] 6.3 Update `docs/design/partner-portal-mvp2-product-design-requirements.md`
  ...
  progression.
- [ ] 6.4 Update affected current-capability purpose text and other user-facing
...
- [ ] 6.5 Update developer documentation and compatibility records for the
  access route map, copy endpoint, legacy adapters, and safe local fixtures.

## 7. Review and verification

- [ ] 7.1 Run focused frontend unit/integration tests and backend service/API/
  authorization tests for every changed scenario.
- [ ] 7.2 Run lint, type checking, production frontend build, backend format/
  lint, OpenAPI export verification, and the repository's relevant fast and
  local verification checks.
- [ ] 7.3 Run automated accessibility checks plus focused keyboard and screen-
  reader review of hubs, tables, row links, forms, errors, menus, dismissal,
  and focus return.
- [ ] 7.4 Capture fake-data desktop, mobile, intermediate-width,
...
- [ ] 7.5 Perform targeted GC Design System, accessibility, bilingual,
  branding, privacy/security, IAM, and information-management review; resolve
  findings or record bounded remaining risk.
- [ ] 7.6 Run a holistic whole-change QA review and confirm requirement,
  scenario, implementation, test, and documentation traceability.
- [ ] 7.7 Archive only after implementation and verification are complete so
...
- [ ] 8.2 Keep production out of scope until explicit approval and release-
