...
**Reason**: The `/mine` route and historical owner checks are retired.
Preflight availability is derived from the caller's active canonical workspace
grant.

**Migration**: Use Grant-derived accessible RP application department
preflight endpoint. The successful, unset-department, unavailable-caller, and
missing-resource scenarios remain covered with
`AccessibleRPApplicationSummaryRead` and safe `404` behavior.

### Requirement: Owner-scoped one-time department assignment endpoint

**Reason**: Department configuration authority belongs to RP Admin and RP User
(Edit) in the owning workspace, not to an owner snapshot.

**Migration**: Use Grant-authorized one-time department assignment endpoint.
The successful assignment, unknown-department, and already-assigned conflict
scenarios are preserved with `AccessibleRPApplicationDepartmentAssignRequest`
and `AccessibleRPApplicationSummaryRead`; the Read Only denial is now explicit.

### Requirement: Protected owner child routes enforce missing-department conflict

**Reason**: Protected child resources use the grant-derived `/accessible`
route family and no longer rely on owner scope.

**Migration**: Use Protected accessible application child routes enforce
missing-department conflict. The OAuth setup and MAU report conflict scenarios
remain covered after safe active-workspace resolution.

### Requirement: Access and error behavior parity for setup route

**Reason**: Owner/non-owner parity is replaced by explicit canonical role and
active-workspace scope behavior.

**Migration**: Use Setup route preserves canonical active-workspace access
behavior. The denied-route and missing-resource paths remain covered while
out-of-scope resources use safe `404 not_found` and Read Only receives no
