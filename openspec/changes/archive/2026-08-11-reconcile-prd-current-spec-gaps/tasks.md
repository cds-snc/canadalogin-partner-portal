...
- [x] 1.2 Record PRD corrections if the intended product scope is narrower than the current PRD text.
	Progress note (2026-08-11): no additional narrowing edit was required in `docs/plans/partner-portal-onboarding-prd.md`. The remaining dashboard-summary and invitation scope stayed intended product behavior and is now represented by archived follow-on changes plus current specs rather than by keeping this reconciler change open.
...
- [x] 4.3 Archive this change only after the PRD is corrected or the required follow-on implementation changes have been created and current specs remain accurate.
	Progress note (2026-08-11): `restore-dashboard-summary-surface` and `restore-external-developer-invitations` have both been archived into current specs, and `openspec/specs/partner-portal-access-and-dashboard/spec.md` now carries the dashboard-summary and invitation-aware current-user surface requirements that this reconciler change handed off.
