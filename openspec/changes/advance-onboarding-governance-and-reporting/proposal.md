...
- Dependency assumption: current workspace, application-information, and workspace-scoped RP application baselines are treated as shipped behavior for this change. Only dashboard-summary or invitation-specific gaps from [openspec/changes/reconcile-prd-current-spec-gaps](../reconcile-prd-current-spec-gaps/proposal.md) remain relevant when a slice explicitly depends on those surfaces.
