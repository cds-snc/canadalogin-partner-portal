...
- [ ] 1.2 Keep the lifecycle vocabulary `draft`, `submitted`, `under_review`, `approved`, and `launched`, and record the actor and rule set for each state transition, including `test`-optional entry, self-serve `test` to `staging`, allowed RP creation for `test` and `staging`, and platform-admin-reviewed `staging` to `production` progression.
- [ ] 1.3 Define backend DTO, persistence, and API contract updates needed to expose onboarding state and lifecycle timestamps on list and detail surfaces, plus target-environment, promotion-status, external-reference, reviewing actor or team, and timestamp fields where progression tracking applies.
...
- [ ] 2.1 Confirm which application information sections, contacts, checklist items, and external evidence references are shown as advisory readiness inputs before submission or production progression.
- [ ] 2.2 Define the completion-indicator behavior for incomplete sections, overall readiness, and missing checklist or evidence-reference context without turning those gaps into hard in-portal gates for MVP2.
...
- [ ] 2.4 Define UI and API behavior for submission or progression attempts when onboarding data, checklist items, or external evidence references are incomplete, while keeping those gaps externally gated for MVP2.
...
- [ ] 3.5 Define review-note and checklist behavior, including platform-admin visibility and update rights, and which metadata-only surfaces remain visible to CL Admin users without exposing RP secret values.
