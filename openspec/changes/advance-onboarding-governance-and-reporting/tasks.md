...
- [ ] 1.1 Confirm the record types in scope for MVP2 state tracking: workspace, application information, and RP application.
- [ ] 1.2 Keep the lifecycle vocabulary `draft`, `submitted`, `under_review`, `approved`, and `launched`, and record the actor allowed to move each state transition.
- [ ] 1.3 Define backend DTO, persistence, and API contract updates needed to expose onboarding state and lifecycle timestamps on list and detail surfaces.
...
- [ ] 2.1 Confirm which application information sections and contacts are required before a record is considered submit-ready.
- [ ] 2.2 Define the completion-indicator behavior for incomplete sections and overall readiness.
...
- [ ] 2.4 Define validation and API behavior for submission attempts when required onboarding data is incomplete.
...
- [ ] 3.3 Define queue filters, list columns, row actions, and record-detail context for the cross-workspace review backlog.
- [ ] 3.4 Define PAT-023 table behavior for the review queue, including loading, empty, error, unauthorized, and pagination states.
- [ ] 3.5 Define reviewer note and checklist behavior, including who can view, add, and update review artifacts.
- [ ] 3.6 Add backend and frontend tests for oversight filtering, review-note capture, and access control.

## 4. Role-Boundary Guidance

- [ ] 4.1 Identify the pages or flows where workspace-member versus invited-developer guidance must appear.
- [ ] 4.2 Define English and French guidance copy and confirm parity expectations under STD-017 official-languages expectations.
- [ ] 4.3 Add tests or review fixtures that cover the new help and guidance surfaces.
...
- [ ] 6.4 Keep PRD and OpenSpec terminology aligned if lifecycle-state or reviewer terminology changes during planning.