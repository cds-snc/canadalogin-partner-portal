	...
	useTranslation: (): { t: (key: string) => string } => ({
		t: (key: string): string => {
			const translations: Record<string, string> = {
				...
				"departments.manageAction": "Manage",
...
vi.mock("@/components/layout", () => ({
	...
	}): ReactElement => <nav>{items.map((i) => i.label).join(" > ")}</nav>,
}));
...
describe("DepartmentsPage", () => {
	...
	it("supports modal-driven department management", () => {
		...
		expect(
			screen.getAllByRole("heading", { name: /departments/i }).length
		).toBeGreaterThan(0);
		expect(
			screen
				.getAllByText(/abbreviation|department name|created/i)
				.map((node) => node.textContent)
		).toEqual(["Abbreviation", "Department name"]);
		fireEvent.click(screen.getByRole("button", { name: /create department/i }));
		expect(
			screen.getByRole("heading", { name: /create department/i })
		).toBeTruthy();
