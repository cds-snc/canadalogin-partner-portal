...
import { WorkspaceApplicationUsagePage } from "@/features/workspaces/pages/WorkspaceApplicationUsagePage";
	...
	useTranslation: (): { t: (key: string, options?: Record<string, unknown>) => string } => ({
		t: (key: string, options?: Record<string, unknown>): string => {
			const translations: Record<string, string> = {
				...
				"workspaces.applicationsAuditMoreAvailable": "More audit events are available for the selected day.",
				"workspaces.applicationsAuditOriginColumn": "Origin",
				"workspaces.applicationsAuditResultColumn": "Result",
				"workspaces.applicationsAuditSummary": "Review bounded audit activity for this workspace-scoped RP application.",
				...
				"workspaces.applicationsUsageSummary": "Review the usage totals for this workspace-scoped RP application on a selected day.",
...
vi.mock("@/components/ui", () => ({
	Button: ({ children, onGcdsClick, type }: PropsWithChildren<{ onGcdsClick?: () => void; type: string }>): ReactElement =>
		type === "button" ? (
	...
	DataTable: ({ rows }: { rows: Array<{ user: string }> }): ReactElement => (
		<section>{rows.map((row, index) => <div key={`${row.user}-${index}`}>{row.user}</div>)}</section>
	),
	DateInput: ({ legend, onInput, value }: { legend: string; onInput?: (event: { target: { value: string } }) => void; value?: string }): ReactElement => (
		<label>
			<span>{legend}</span>
			<input
				value={value}
...
vi.mock("@/features/workspaces/hooks/use-workspace-rp-applications", () => ({
	...
	useWorkspaceRPApplicationUsageSummary: vi.fn(),
}));

describe("Workspace application operations pages", () => {
	it("renders the usage summary totals", () => {
		...
		vi.mocked(useWorkspaceRPApplicationUsageSummary).mockReturnValue({
			error: null,
			isLoading: false,
			refetch: vi.fn((): Promise<unknown> => Promise.resolve()),
			summary: { failed: 2, succeeded: 9, total: 11 },
		});
		...
		render(<WorkspaceApplicationUsagePage />);

		expect(
			screen.getByRole("heading", { name: /usage summary - benefits portal/i })
		).toBeTruthy();
	...
	it("shows the usage conflict detail when telemetry is unavailable", () => {
		vi.mocked(useWorkspaceRPApplication).mockReturnValue({
			application: {
				created_at: "2026-07-31T10:05:00Z",
				created_by: 7,
		...
		vi.mocked(useWorkspaceRPApplicationUsageSummary).mockReturnValue({
			error: new ConflictRequestError({
				detail:
					"Link the RP application to an IBM Security Verify application before viewing usage telemetry.",
			}),
			isLoading: false,
			refetch: vi.fn((): Promise<unknown> => Promise.resolve()),
			summary: null,
		...
		expect(
			screen.getByText(
				/link the rp application to an ibm security verify application before viewing usage telemetry/i
			)
		).toBeTruthy();
	});
	...
	it("renders audit results and loads more events", () => {
		...
		vi.mocked(useWorkspaceRPApplicationUsageSummary).mockReturnValue({
			...
			isLoading: false,
			refetch: vi.fn((): Promise<unknown> => Promise.resolve()),
			summary: { failed: 0, succeeded: 0, total: 0 },
		});
		vi.mocked(useWorkspaceRPApplicationAuditTrail).mockReturnValue({
			error: null,
			events: [
