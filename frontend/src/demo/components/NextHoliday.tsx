...
const NextHoliday: React.FC<NextHolidayProps> = ({
	...
	const calcNextHoliday = (dateString: string) => {
		...
		return Math.floor(
			(holidayDate.getTime() - today.getTime()) / (1000 * 3600 * 24)
		);
	};
	...
	const getObservedInProvinces = () => {
		if (provincesObservedIn) {
			if (provincesObservedIn.length === 1) {
				return (
					<abbr title={provincesObservedIn[0].nameEn}>
						{provincesObservedIn[0].id}
					</abbr>
				);
			} else {
				return provincesObservedIn.map((value, i: number) => (
					<span key={i}>
						{i === provincesObservedIn.length - 1 && " and "}
						<abbr title={value.nameEn}>{value.id}</abbr>
						{i != provincesObservedIn.length - 1 ? ", " : "."}
					</span>
				));
			}
		}
	...
	const daysToNextHoliday = nextHoliday
		? calcNextHoliday(nextHoliday.date)
		: null;
	...
	return display === "banner" ? (
		<div className="d-flex bg-primary md:align-items-center align-items-start text-light mb-450 md:px-450 px-250 py-200">
			...
			<Text textRole="light" marginBottom="0">
				<strong>
					Next holiday is {nextHoliday.nameEn} — that's {daysToNextHoliday}{" "}
					{dayOrDays} away
				</strong>
			</Text>
		</div>
		...
		<section
			className={`pt-700 pb-700 mb-300 next-holiday-homepage ${federal ? `img-federal` : "img-nonfederal"}`}
		>
			<div className="bg-light md:px-450 px-250 d-block pt-100 pb-500">
				...
				<strong className="d-block mb-100 font-h4">{nextHoliday.nameEn}</strong>
				<div className="font-h5 font-medium">
					<time>{formatDate(nextHoliday.date)}</time>
					{!federal ? (
			...
			<div className="d-flex bg-primary md:align-items-center align-items-center text-light md:px-450 px-250 py-200">
				<Text textRole="light" marginBottom="0">
					<strong>
						That's in {daysToNextHoliday} {dayOrDays}!
					</strong>
				</Text>
				<img
