...
} from "@/demo/data/tableTestSubmissionsData";
...
const Table: React.FC = () => {
	...
	return (
		<section>
			...
			<Text>
				Government export certificate application table. Approve or reject
				pending submissions.
			</Text>
				...
				<thead>
					<tr>
						...
						{Object.entries(tableTestSubmissionColumns).map(([key, label]) => (
							<th key={key} tabIndex={0} className="focusable-th">
								{label}
							</th>
						))}
						<th>Actions</th>
				...
				<tbody>
					{tableTestSubmissionData.map((row, idx) => (
						<tr key={idx}>
							...
							{/*</td>*/}
							<td>
								<Link to={`/table-gridjs#${row.submission_id}`}>
									{row.submission_id}
								</Link>
							</td>
							<td>{row.submitter_name}</td>
							<td>
								{new Date(row.date_submitted).toLocaleDateString("en-CA")}
							</td>
							<td>
								<StatusPill
									status={
