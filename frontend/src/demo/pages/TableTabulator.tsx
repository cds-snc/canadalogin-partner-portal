...
const TableTabulator: React.FC = () => {
	...
	useEffect(() => {
		...
		if (tableDiv) {
			...
			const columns = Object.entries(tableTestSubmissionColumns).map(
				([field, title]) => {
			...
			const tabInstance = (
				tableDiv as unknown as { _tabulator?: { destroy: () => void } }
			)._tabulator;
			if (tabInstance) {
				tabInstance.destroy();
			}
	...
	return (
		<section>
			...
			<div
				ref={tableRef}
				style={{ width: "100%", margin: "2em 0", position: "relative" }}
			>
				<span
					style={{
						position: "absolute",
