...
const ViewHolidaysNationwide = () => {
	...
	useEffect(() => {
		...
		axios
			.get<{ holidays: Holiday[] }>(endpointForYear)
			.then(({ data }) => {
	...
	const mapHolidayData = (holidays: Holiday[]): MappedHoliday[] => {
		const holidayMap = new Map<
			string,
			{ nameEn: string; provinces: string[]; federal: boolean }[]
		>();
		...
		holidays.forEach((holiday) => {
			...
			const existingHoliday = holidayMap
				.get(date)
				?.find((h) => h.nameEn === nameEn);
	...
	const formatLocation = (provinces: string[], federal: boolean): string => {
		...
		location += provinces.length
			? (location ? ", " : "") +
				provinces
					.map((province) => (province === "PE" ? "P.E.I." : province))
					.join(", ")
			: "";
			...
			<table className="mb-450">
				<thead>
					<tr className="text-left bb-sm b-default">
						...
						<th className="py-225 font-h4">Location</th>
					</tr>
				</thead>
				<tbody>
