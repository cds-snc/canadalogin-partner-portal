...
export const ApplicationInformationDetailsEditPage = (): FunctionComponent => {
	...
	const { applicationInformationUuid, workspaceUuid } = useParams({
		from: "/workspaces/$workspaceUuid/applications/$applicationInformationUuid/details/edit",
	});
	const {
	...
	const sourceUuid = applicationInformation?.uuid ?? applicationInformationUuid;
	const form: ApplicationInformationFormState = {
		...createEmptyApplicationInformationForm(),
		...(applicationInformation
			? toApplicationInformationFormState(applicationInformation)
			: {}),
		...(formDraft?.sourceUuid === sourceUuid ? formDraft.values : {}),
	};
	...
	const handleUpdate = async (): Promise<void> => {
		...
		try {
			const updatedApplication = await updateApplicationInformation(
				workspaceUuid,
				applicationInformationUuid,
			...
			await navigate({
				params: {
					applicationInformationUuid: updatedApplication.uuid,
					workspaceUuid,
				},
				replace: true,
				search: { updated: "1" },
		...
		<>
			...
			{applicationInformation ? (
				<ApplicationInformationForm
					cancelHref={`/workspaces/${workspaceUuid}/applications/${applicationInformationUuid}/details`}
					form={form}
					...
					onChange={(field, value) => {
						setFormDraft((currentDraft) => ({
							sourceUuid,
							values: {
								...(currentDraft?.sourceUuid === sourceUuid
									? currentDraft.values
									: {}),
								[field]: value,
							},
