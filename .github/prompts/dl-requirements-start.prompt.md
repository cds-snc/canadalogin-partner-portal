---
id: dl-requirements-start
description: Start an active OpenSpec change package from a rough brief, requirements note, issue, or pasted discovery notes.
use_when:
  - No active OpenSpec change exists yet for the work.
  - A rough brief, requirements note, issue, or discovery note should become the first OpenSpec change package.
  - The agent should pick a safe first slice and create proposal, design, tasks, and spec delta files.
...
common_overlays:
  ...
  - accessibility
required_inputs:
  - source_brief_or_requirements
  - desired_change_id_when_known
  - known_users_or_systems
  - constraints
  - target_work_context_when_not_local
produces:
  - active_openspec_change_package
...
Use this prompt when the user is ready to start OpenSpec artifacts and wants the agent to choose a small, local-first slice. Use `dl-requirements-shape` instead when the intent is still too unclear to choose a change, capability, or first slice.
...
7. Use [.github/skills/delorean-question-resolution/SKILL.md](../skills/delorean-question-resolution/SKILL.md) to resolve discoverable questions from repo guidance, OpenSpec, `architecture_docs`, code, tests, contracts, and approved docs before asking the user.
8. Use [.github/skills/delorean-openspec/SKILL.md](../skills/delorean-openspec/SKILL.md) to create the OpenSpec package.
