...
class TestOidcService:
    ...
    @pytest.mark.asyncio
    async def test_callback_stores_logout_context_and_indexes_session(self, mock_db):
        service = OidcService()
        service.logout_service = Mock()
        service.logout_service.store_session = AsyncMock()
        request = Mock(session={})
        claims = {
            "sub": "subject-123",
            "email": "oidc.user@example.com",
        ...
        with patch("src.app.services.oidc_service.get_oidc_client", return_value=client):
            with patch("src.app.services.oidc_service.sync_oidc_user", new_callable=AsyncMock) as mock_sync:
                with patch("src.app.services.oidc_service.regenerate_session_id", create=True):
                    with patch("src.app.services.oidc_service.get_session_id", return_value="session-123", create=True):
                        mock_sync.return_value = oidc_user
        ...
        service.logout_service.store_session.assert_awaited_once_with("sid-123", "session-123")
    ...
    async def test_backchannel_logout_removes_matching_local_session(self):
        ...
        logout_service.get_session_id = AsyncMock(return_value="session-123")
        ...
        logout_service.remove_local_session.assert_awaited_once_with("session-123")
    ...
    async def test_backchannel_logout_rejects_invalid_logout_token(self):
        ...
        logout_service.get_session_id = AsyncMock()
        logout_service.remove_local_session = AsyncMock()
        logout_service.remove_session = AsyncMock()
        service = OidcService(logout_service=logout_service)
        ...
        logout_service.get_session_id.assert_not_awaited()
        logout_service.remove_local_session.assert_not_awaited()
        logout_service.remove_session.assert_not_awaited()
