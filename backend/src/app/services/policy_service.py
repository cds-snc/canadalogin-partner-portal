...
from ..repositories.crud_access_policies import crud_access_policies
...
class PolicyService:
    ...
    async def update_policy(
        ...
        # crud.exists returns a boolean or an object in different implementations; handle both
        if isinstance(matching_policy, dict):
            if str(matching_policy.get("uuid")) != str(policy_uuid):
                raise DuplicateValueException("Policy already exists")
        elif matching_policy:
            # boolean True indicates an existing policy (unable to compare uuid)
            raise DuplicateValueException("Policy already exists")
            raise DuplicateValueException("Policy already exists")
