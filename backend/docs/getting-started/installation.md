...
1. **Install Python dependencies**:
   ...
   This installs the runtime plus development tools into the repo-root `.venv`, which is also what the top-level `Makefile` uses for backend commands.
...
1. **Start the application**:
   ...
   ```bash
   UV_PROJECT_ENVIRONMENT=../.venv uv run uvicorn src.app.main:app --reload --host 0.0.0.0 --port 8000
   ```
...
1. **Start the worker** (in another terminal):
   ...
   ```bash
   UV_PROJECT_ENVIRONMENT=../.venv uv run arq src.app.core.worker.settings.WorkerSettings
   ```
