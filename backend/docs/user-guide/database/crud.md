...
This guide covers the database-oriented repository adapters used in the backend. These adapters are implemented with FastCRUD and live under `src/app/repositories/`, which is the main data-access layer for both database persistence and external clients.
...
Each database-backed model has a corresponding repository adapter module that defines the available operations:

```python
# src/app/repositories/crud_users.py
...
```python
# From src/app/services/user_service.py - checking before creating
...
```python
# From service code - check before updating
