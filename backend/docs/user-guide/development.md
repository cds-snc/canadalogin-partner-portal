...
#### 3. Create Repository Adapter Operations

Create `src/app/repositories/crud_categories.py`:
...
from ...repositories.crud_categories import crud_categories
...
# Run with coverage
cd backend && UV_PROJECT_ENVIRONMENT=../.venv uv run pytest --cov=src

# Run specific test file
cd backend && UV_PROJECT_ENVIRONMENT=../.venv uv run pytest tests/test_api.py

# Run with verbose output
cd backend && UV_PROJECT_ENVIRONMENT=../.venv uv run pytest -v

# Run tests matching pattern
cd backend && UV_PROJECT_ENVIRONMENT=../.venv uv run pytest -k "test_user"
```
...
UV_PROJECT_ENVIRONMENT=../../.venv uv run alembic revision --autogenerate -m "Description of changes"
...
cd src
UV_PROJECT_ENVIRONMENT=../../.venv uv run alembic revision --autogenerate -m "Add categories table"
UV_PROJECT_ENVIRONMENT=../../.venv uv run alembic upgrade head
```
...
UV_PROJECT_ENVIRONMENT=../../.venv uv run alembic revision --autogenerate -m "Add bio field to users"
